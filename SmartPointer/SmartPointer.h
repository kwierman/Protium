...
namespace Protium{
    ...
    namespace SmartPointer{
        ...
        class HeapPointer{
            Pointee* fPointee;
        public:
            typedef Pointee* StoredType;
            typedef Pointee* InitPointerType;
            typedef Pointee* PointerType;
            typedef Pointee& ReferenceType;

            HeapPointer() : fPointee(Default()) {}

        // The storage policy doesn't initialize the stored pointer
        //     which will be initialized by the OwnershipPolicy's Clone fn
        HeapPointer(const HeapPointer&) : fPointee(0)
        {}

        template <class U>
        HeapPointer(const HeapPointer<U>&) : fPointee(0)
        {}

        HeapPointer(const StoredType& p) : fPointee(p) {}

        PointerType operator->() const { return fPointee; }

        ReferenceType operator*() const { return *fPointee; }

        void Swap(HeapPointer& rhs)
        { std::swap(fPointee, rhs.fPointee); }

        // Accessors
        template <class F>
        friend typename HeapPointer<F>::PointerType GetImpl(const HeapPointer<F>& sp);

        template <class F>
        friend const typename HeapPointer<F>::StoredType& GetImplRef(const HeapPointer<F>& sp);

        template <class F>
        friend typename HeapPointer<F>::StoredType& GetImplRef(HeapPointer<F>& sp);
    ...
    template <class T>
    inline typename HeapPointer<T>::PointerType GetImpl(const HeapPointer<T>& sp)
    { return sp.pointee_; }

    template <class T>
    inline const typename HeapPointer<T>::StoredType& GetImplRef(const HeapPointer<T>& sp)
    { return sp.pointee_; }

    template <class T>
    inline typename HeapPointer<T>::StoredType& GetImplRef(HeapPointer<T>& sp)
    { return sp.pointee_; }
